...
import { logTrackingStack, beginUntrackFrame, endUntrackFrame, valueForTag, beginTrackFrame, consumeTag, endTrackFrame, validateTag, createTag, dirtyTag, untrack, deprecateMutationsInTrackingTransaction, CONSTANT_TAG, isTracking, tagFor, setTrackingTransactionEnv, createUpdatableTag, CURRENT_TAG } from '@glimmer/validator';
...
class Input extends InternalComponent {
  constructor() {
    super(...arguments);
    this.modernized = Boolean(false
    /* EMBER_MODERNIZED_BUILT_IN_COMPONENTS */
    );
    this._checked = valueFrom(this.args.checked);
    this._value = valueFrom(this.args.value);
  }
  /**
  ...
  get checked() {
    if (this.isCheckbox) {
      warn('`<Input @type="checkbox" />` reflects its checked state via the `@checked` argument. ' + 'You wrote `<Input @type="checkbox" @value={{...}} />` which is likely not what you intended. ' + 'Did you mean `<Input @type="checkbox" @checked={{...}} />`?', untrack(() => this.args.checked !== undefined || this.args.value === undefined || typeof valueForRef(this.args.value) === 'string'), {
  ...
  elementFor(event) {
    assert('[BUG] Event target must be the <input> element', event.target instanceof HTMLInputElement);
    return event.target;
  }
  ...
  callbackFor(type) {
    ...
    if (callback) {
      assert(`The \`@${type}\` argument to the <Input> component must be a function`, typeof callback === 'function');
      return callback;
    } else {
      return NOOP$2;
    }
...
__decorate([action], Input.prototype, "checkedDidChange", null);

__decorate([action], Input.prototype, "valueDidChange", null);

__decorate([action], Input.prototype, "change", null);

__decorate([action], Input.prototype, "input", null);

__decorate([action], Input.prototype, "keyUp", null); // Deprecated features
